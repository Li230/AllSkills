...
### 自建工作流与质量约束（opsx-flow 系列，单文件夹）
- [opsx-flow](opsx-flow/SKILL.md) - 编排器总览（六阶段 + 调度表 + 恢复机制）
- [new](opsx-flow/new/SKILL.md) - 变更脚手架（生成变更目录 + .openspec.yaml + _checkpoint.md 执行契约）
- [explore](opsx-flow/explore/SKILL.md) - Phase 1 需求探索与质询（grill-me）
- [proposal](opsx-flow/proposal/SKILL.md) - Phase 2 方案构建（必读 brainstorming）
- [plan](opsx-flow/plan/SKILL.md) - Phase 3 原子任务规划（design.md + tasks.md）
- [apply](opsx-flow/apply/SKILL.md) - Phase 4 执行与调度（按任务类型调对应 skill）
- [verify](opsx-flow/verify/SKILL.md) - Phase 5 验证与换模型审查
- [archive](opsx-flow/archive/SKILL.md) - Phase 6 归档收尾（archive/ + STATUS.md 更新）
